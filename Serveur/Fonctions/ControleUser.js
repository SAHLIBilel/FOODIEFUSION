export function ControleUser(user){
    let verif=true;
    return ((verifMail(user.email)==='green')&&(verifPassword(user.password)==='green')&&(verifName(user.nom)==='green')&&(verifName(user.prenom)==='green')&&(verifCIN(user.CIN)==='green')&&(verifNumber(user.telephone)==='green'))
}
...
export function verifCIN(Num) {
    ...
    if ((Num.toString()).length==8 && (F_Num === '0' || F_Num === '1')) {
        return "green";
    } else {
        return "red";
    }
